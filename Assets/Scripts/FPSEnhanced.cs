...
public class FPSEnhanced : MonoBehaviour
{
    ...
    [SerializeField]
    private float m_duckingSpeed = 2.0f;
    ...
    private bool m_ToggleDuck = false;
    ...
    private bool goDucking;
    private float duckingTime = 0.4f;
    ...
    private void Start()
    {
        ...
        InputManager.Instance.Input_Duck += InputDucking;
    }

    private void InputDucking()
    {
        if (goDucking) return;
        RaycastHit hit;
        if (Physics.Raycast(transform.position, Vector3.up, transform.localScale.y * 0.9f - m_Controller.center.y) && m_ToggleDuck)
        {
            Debug.Log("something above cant raise up ");
            return;
        }
        m_ToggleDuck = !m_ToggleDuck;
        m_Speed = m_ToggleDuck ? m_duckingSpeed : m_WalkSpeed;
        StartCoroutine(Ducking());

    }
    private IEnumerator Ducking()
    {
        goDucking = true;
        var height = m_ToggleDuck ? 0.9f : 1.8f;
        
        var center = new Vector3(0, m_Controller.center.y,0);
        DOTween.To(() => center, x => center = x, new Vector3(0, height * -0.333333333333f, 0), 0.4f);
        DOTween.To(() => m_Controller.height, x => m_Controller.height = x, height, 0.4f);
        while (m_Controller.height != height)
        {
    ...
    private void Update()
    {
        Debug.DrawRay(transform.position, Vector3.up * transform.localScale.y * 0.9f  - (Vector3.up * m_Controller.center.y));
        transform.rotation *= Quaternion.AngleAxis(InputManager.Instance.CameraMovementX * m_rotationSpeed * Time.deltaTime, transform.up);
    }
    ...
    private void FixedUpdate()
    {
        ...
        if (m_Grounded)
        {
            ...
            if (!m_ToggleDuck)
            {
                m_Speed = Input.GetKey(KeyCode.LeftShift) ? m_duckingSpeed : m_WalkSpeed;
            }
